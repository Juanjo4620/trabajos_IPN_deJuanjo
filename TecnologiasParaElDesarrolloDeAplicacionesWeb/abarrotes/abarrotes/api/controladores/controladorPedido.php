...
class ControladorPedido {
  ...
  public static function mostrar($id) {
    ...
    // POST /api/pedidos/{pedidoId}/items/{detalleId}/recibido
  public static function marcarItemRecibido($pedidoId, $detalleId) {
    $usuario = Autenticacion::usuarioDesdeSolicitud();
    if (!$usuario) { http_response_code(401); echo json_encode(['error'=>true,'mensaje'=>'No autorizado']); return; }

    $pid = (int)$pedidoId; $did = (int)$detalleId;
    if ($pid<=0 || $did<=0) { http_response_code(400); echo json_encode(['error'=>true,'mensaje'=>'IDs inválidos']); return; }

    $esTiendero = (($usuario['rol'] ?? '') === 'tiendero');

    try {
      Pedido::marcarItemRecibido($pid, $did, (int)$usuario['id'], $esTiendero);
      echo json_encode(['mensaje'=>'Item marcado como recibido']);
    } catch (InvalidArgumentException $e) {
      http_response_code(400); echo json_encode(['error'=>true,'mensaje'=>$e->getMessage()]);
    } catch (RuntimeException $e) {
      http_response_code(422); echo json_encode(['error'=>true,'mensaje'=>$e->getMessage()]);
    } catch (Throwable $e) {
      http_response_code(500); echo json_encode(['error'=>true,'mensaje'=>'Error interno']);
    }
  }

  // POST /api/pedidos/{pedidoId}/items/{detalleId}/devolucion
  public static function solicitarDevolucionItem($pedidoId, $detalleId) {
    $usuario = Autenticacion::usuarioDesdeSolicitud();
    if (!$usuario) { http_response_code(401); echo json_encode(['error'=>true,'mensaje'=>'No autorizado']); return; }

    $entrada = json_decode(file_get_contents('php://input'), true) ?? [];
    $motivo = isset($entrada['motivo']) ? trim((string)$entrada['motivo']) : null;
    ...
    try {
      Pedido::solicitarDevolucionItem($pid, $did, (int)$usuario['id'], $esTiendero, $motivo);
      echo json_encode(['mensaje'=>'Devolución solicitada']);
    } catch (InvalidArgumentException $e) {
      http_response_code(400); echo json_encode(['error'=>true,'mensaje'=>$e->getMessage()]);
    } catch (RuntimeException $e) {
      http_response_code(422); echo json_encode(['error'=>true,'mensaje'=>$e->getMessage()]);
